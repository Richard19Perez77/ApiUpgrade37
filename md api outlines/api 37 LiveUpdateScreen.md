...
1. **API 37+** → Full semantic colors
2. **API 26-36** → Basic ongoing notification (fallback)
3. **API 25-** → Not supported (notification channels not available)
...
1. **Semantic Colors** → Text with meaning (green = safe, red = danger)
2. **Universal Language** → Same colors across all apps
3. **Promoted Ongoing** → Important notifications are visible
4. **Stricter Limits** → Better system health
5. **Platform Preferences** → Easier for developers
