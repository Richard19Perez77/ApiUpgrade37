 ...
 *
 * - Need — layouts must reflow. App Bubbles / interactive desktop PiP are niceties your UI also has to survive, but the lock-ignore is the upgrade tax.
 */
@Composable
fun AdaptiveLayoutsScreen(
    ...
    val body = @Composable { padding: PaddingValues ->
        FeatureBody(
            ...
            /**
             * BoxWithConstraints exposes a BoxScope, so sibling children are STACKED on top of each other, not laid out vertically. The readout and the panes need an explicit Column, or they overlap.
             */
    ...
    if (onBack == null) {
        Scaffold { padding ->
            ...
            /**
             * Keep the horizontal insets: in landscape they carry the display cutout and the side navigation bar. Dropping them is the classic edge-to-edge bug, and it would be a poor look on the adaptive-layout screen in particular.
             */
